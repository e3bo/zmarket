...
val_crdat <- vdat[valu$subs, ] %>% ungroup() %>% select(duration:social_proof)
val_crdat$cohort <- valu$cohort

p <- predict(m1, as.data.frame(val_crdat), type = "fitted") # as.data.frame needed because predictrms uses [,i] to select vectors 

m1_validation_logloss <- -sum(log(ifelse(valu$y == 1, p, 1 - p)))
