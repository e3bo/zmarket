...
Next we will put the overall effects of the variables in perspective with each other and with the other variables in the model. We will use the sum of the absolute values of all effects of the levels of a variable as a measure of varible importance. 

```{r fig.width = 10, fig.heigh = 17}


pdata %>% 
  ggplot(aes(x = var, y = abs(est), fill = paste(var, level, sep = "="))) + 
  geom_col() + 
  scale_fill_discrete(name = "Answer level") + 
...
cdf <- data.frame(ncoefficients = -ests7[,1], vname = rownames(ests7))
...
cdf2 <- cdf %>% mutate(var_type =
                       case_when(stringr::str_detect(vname, "^offer|^price|^social_proof|^rtb|^outcome|^duration") ~ "attribute",
...
cdf2 %>% 
  ...
  select(Variable=vname, "Effect on logit of increasing rating"=ncoefficients) %>% 
