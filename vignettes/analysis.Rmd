...
rstats <- wdat %>% group_by(response_id) %>% summarise(mean_ans = mean(answer), sd_ans = sd(answer)) 
...
The mean response appears to follow a mixture distribution. A certain fraction of about 250 / 892 of respondents answered 1 ("Very unlikely") in all 12 tasks. Another 70 or so respondents also provided the same answer to all tasks, resulting in 320 respondents with an standard deviation of zero. Models for these groups of constant-response individuals could be very simple, if they could be accurately identified. On balance the mean answers from the respondents that did not always answer 1 seem to be uniformly distributed. These individuals may be responding in an interesting way to some attributes in the experiment.
